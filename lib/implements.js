...
function implementsInterface(actual, expected, opts) {
	...
	// Creates instance of given constructor without executing it.
	function instanciateConstructor(original) {
		var noob = function() {};
		inherits(noob, original);
		return new noob();
	}

	opts = _.defaults(opts || {}, {
		instanciateConstructors: true,
	...
	if(opts.instanciateConstructors) {
		// Prototype have to be instanciated before checking
		expected = typeof expected === "function" ? instanciateConstructor(expected) : expected;
	}
